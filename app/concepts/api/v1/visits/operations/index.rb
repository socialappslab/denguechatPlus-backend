    ...
    module Visits
      module Operations
        class Index < ApplicationOperation
          ...
          def validate_schema
            @ctx['contract.default'] = Api::V1::Visits::Contracts::Index.kall(@params)
          ...
          def cursor_and_paginate
            @ctx[:sort] = { field: 'visits.visited_at', direction: 'desc' } if @params['sort'].nil?
            direction = @params['order'].nil? ? 'asc' : @params['order']
            @ctx[:sort] = {field: @params['sort'].underscore, direction: } if @params['sort']
          end

          def list
            @ctx[:data] = Api::V1::Visits::Queries::Index.call(@ctx['contract.default']['filter'], @ctx[:sort])
