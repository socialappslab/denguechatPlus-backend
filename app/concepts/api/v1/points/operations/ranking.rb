...
module Api
  module V1
    module Points
      module Operations
        class Ranking < ApplicationOperation
          ...
          def validate_schema
            @ctx['contract.default'] = Api::V1::Points::Contracts::Ranking.kall(@params)
          ...
          def cursor_and_paginate
            @ctx[:sort] = { field: 'total_points', direction: 'desc' } if @params['sort'].nil?
            direction = @params['order'].nil? ? 'desc' : @params['order']
            @ctx[:sort] = { field: @params['sort'], direction: } if @params['sort']
          end

          def list
            @ctx[:data] = Api::V1::Points::Queries::Ranking.call(@ctx['contract.default']['filter'], @ctx[:sort])
